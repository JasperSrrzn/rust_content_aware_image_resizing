...
fn main() {
    let image = ImageWrapper::from_file("./images/france.jpg");
    let energy = image.get_energy();
    let result = get_minimum_seam(&image, &energy);
    let mut im = image.get_image().clone();
    result.iter().for_each(|(x, y)| {
        im.put_pixel(*x, *y, Rgba::from([255, 0, 0, 255]))
    });
    let image_result = im.save("./images/seam.jpg");
    match image_result {
        Ok(_) => {}
        Err(e) => panic!("{}", e)
    }
